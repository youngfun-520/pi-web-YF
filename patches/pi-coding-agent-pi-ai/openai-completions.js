...
export const streamOpenAICompletions = (model, context, options) => {
    ...
    (async () => {
        ...
        try {
            ...
            // Fix: Some models (e.g., DeepSeek with thinking enabled) send the response
            // entirely as reasoning_content, leaving content empty. This results in only
            // a thinking block with no text block, hiding the response behind a collapsed
            // "Thinking" section in the UI. Promote thinking content to text as fallback.
            if (thinkingBlock && thinkingBlock.thinking.trim().length > 0 &&
                (!textBlock || textBlock.text.trim().length === 0)) {
                const fallbackText = { type: "text", text: thinkingBlock.thinking };
